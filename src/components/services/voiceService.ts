...
export const handleVoiceInteraction = async (
  lang: string = "hi",
  addMessageToChat?: (message: { role: string; text: string }) => void
) => {
  try {
    if (!isRecording) {
      ...
      // ✅ Add only the answer text to chat
      if (addMessageToChat) {
        addMessageToChat({ role: "assistant", text: botAnswer });
      }

      const mappedLang = LANGUAGE_MAP[lang] || "english";
      const ttsResult = await textToSpeech(botAnswer, mappedLang);
